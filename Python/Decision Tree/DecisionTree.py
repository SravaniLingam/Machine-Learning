...
def entropy(rows):
    ...
    for lbl in counts:
        ...
        value1 = math.log2(prob_of_lbl)
        impurity -= prob_of_lbl*value1
    return impurity
...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        ...
        self.predicted_class_label = max(self.predictions, key = self.predictions.get)
        self.id=id
        self.depth=depth
...
def get_node_id(node, nodelist=[]):
    if node or isinstance(node, Leaf):
        nodelist.append(node.id)
    if not isinstance(node, Leaf):
        if node.true_branch:
            get_node_id(node.true_branch, nodelist)
        if node.false_branch:
            get_node_id(node.false_branch, nodelist)

    return nodelist
def get_random_node_list(nodeList):
    number_of_nodes = 1
    ret_node_list = []
    k=0
    count=0
    while count != number_of_nodes:
        ...
        if k not in ret_node_list:
            ret_node_list.append(k)
            count += 1
        else:
            pass
...
def print_tree(node, spacing=""):
    ...
    if isinstance(node, Leaf):
        ...
        print(spacing + "Predicted class label", node.predicted_class_label)
        return
